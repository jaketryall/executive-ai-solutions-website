...
## 16. THE PARAGRAPH SWEEP (measured 2026-09-21 — Jake: "the scroll effect on the text … where the words like glow")

`data-anim="paragraph-sweep"` on the 57px paragraph (§15's "Anyone can make…"). Each rendered LINE is a `<span>` with `-webkit-text-fill-color: transparent; background-clip: text; background-repeat: no-repeat; background-size: 300% 100%` and the gradient `linear-gradient(90deg, currentColor 0%, currentColor 40%, var(--color-brand-magenta) 50%, color-mix(in srgb, currentColor 20%, transparent) 60% 100%)`. `background-position` x runs **100% → 0%** — the faint third over the text, then the magenta edge crossing it left to right, then ink — **scrubbed by scroll, both ways** (back to 100% when scrolled back up), `transition: all` on the span smoothing it. Geometry at 1440×900: a line sweeps while its own top passes **~648 → ~435px (.72 → .48 of the screen, ~210px of scroll)**; the next line's sweep starts when the previous is ~40% through (its own top is 52px lower, so the cascade is just each line reaching the same screen band). The nav's links carry the same gradient at rest (a hover sweep). This — not the headline fade — is the "glow": the accent riding the fill's leading edge. Ported: `.dr-sweep-l` (room.css) off the paragraph's one clock, `SweepLines` (services-section.tsx) making the lines.
