...
## §03 · THE WORK v4 · LEOPARPEIX'S RAIL, ON SCROLL (2026-09-18)
- Geometry: each `.dr-work-tile` rect is 1045×604 ±1 (ratio 1.7302), gap between adjacent tiles 20px ±1, at 1440×736 (`--th` must resolve off `41.944vw`, not get capped by the safety `svh` ceiling at this viewport — that ceiling silently bound once, at 70svh, and is now 90svh). With the block's own `--bp` at 0 (settled, its top at the fold): tile 0's rect centre is within ±1 of 720; tile 1 peeks at the right edge, nothing else visible. At `--bp` 1: the LAST tile's rect centre is within ±1 of 720.
- The two parallaxes, read off `getComputedStyle(.dr-work-img).translate`: at `--bp` 0 the Y component is −8.5% of `--th` in PX (−51.3px ±1 at 604), at `--bp` .5 it is ~0, at `--bp` 1 it is +51.3px ±1 — this must be an absolute px calc off `--th`, never a bare `%` (a bare `%` resolves against the img's own 120%-enlarged box, 724.8px, not the tile, and overshoots by ~10px). The X component IS a bare `%` (resolves against the img's own enlarged width by design, matching leoparpeix's own uParallaxX): a centred tile's X ≈ 0; an adjacent tile's `--ndc` × −1.25 × 8.5%, of that tile's own (120%) width. No exposed well edge in the part of any tile actually inside the viewport at `--bp` 0 / .5 / 1 (check `well.left/right` against `img.left/right` only within `max(well.left,0)`–`min(well.right, innerWidth)`; a tile scrolled mostly off-screen can legitimately fail the raw-rect check on its OFF-screen side, since `overflow: clip` on the band removes that part from render entirely).
- Force, at REST (page settled 6-8s, no scrolling since load): `--force` on every `.dr-work-band` reads the exact string `"0.000"`, and every `.dr-work-tile`'s computed `transform` is the flat 2D identity (`matrix(1, 0, 0, 1, 0, 0)`), never a `matrix3d` with a non-zero 15th entry. A real wheel burst (dispatched `WheelEvent`s — Lenis does not gate on `isTrusted`, so a script-dispatched wheel still drives its smoothing, which is what sustains the mark's velocity past the literal dispatch window) of 6×100px over ~120ms: `--force` crosses 0.5 within ~300ms of the burst's start and decays back under 0.02 within 1.8s of the LAST wheel dispatch. While it is up, an off-centre tile's rect width is measurably smaller than a centred tile's (e.g. 759px vs 1045px at a 0.56 peak); at rest both read 1045 ±1. (A raw `scrollBy`-only burst, with no Lenis momentum to sustain it, tops out lower — ~0.46 — and is not itself a regression; it just isn't the recipe that clears this line.)
- Coast: settled mid-passage, then one `scrollBy(0, 300)` — `--bp` is still changing 100ms later, and has stopped changing (two consecutive equal reads) by ~500–700ms (the 0.1 chase).
- Pill on tile 0 of each block (needs a REAL pointer hover, not a dispatched `mouseover`): within 700ms every `.dr-char` inside `.dr-work-see` computes a `transform` matrix whose vertical component equals −(that char's own rendered height) (translateY(−100%)); the pill's own rect is unchanged before/during/after. Un-hover returns every `.dr-char` transform to `none`.
- An 8-step scroll sweep through one block's whole passage: no step leaves any `.dr-work-tile` at zero width/height, `visibility: hidden`, or `opacity: 0`. A settled frame-time probe (`scrollBy(0,14)` × 130, page settled first) shows 0 frames over 34ms.
- At ≤900px width or under `prefers-reduced-motion: reduce`: `.dr-work-band` computed `overflow-x` is `auto` and `scroll-snap-type` is `x mandatory`; `.dr-work-tile` computed `transform` is `none` (so a lingering non-zero `--force` from before the breakpoint changed is harmless — nothing on this layout consumes it); setting `band.scrollLeft` moves it; `.dr-work-info` computed `grid-template-columns` is two tracks (`3ch 1fr`), not five.
- Every `font-size` inside `.dr-work-*` resolves to a `--fs-*` token (`--fs-section`, `--fs-body`, `--fs-body-s`, `--fs-micro`), per the page-wide type-scale law — no new raw px size.
- `.dr-work-h` computes the same `--fs-section` (38.4px at 1440) as the other section headers; `[data-wipe]` on `.dr-work-kicker`/`.dr-work-h` fires (`is-wiped`) once their top crosses <0.9vh and both restore to byte-identical authored text afterward — read the fired state inside the SAME `evaluate_script` round trip as the scroll that triggers it.
