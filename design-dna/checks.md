...
- §01 THE RELOAD (scripts/reload-screencast.mjs, then PIL per-region means): the FIRST painted frame is the light room (frame mean ≳120, never the dark room's ~34); the film region goes from the ground (~230) MONOTONICALLY down to the film (~30) as the reel rises in — any bounce back up mid-way is a crossfade letting the ground through, any dark value before ~500ms is a plate that was later removed; the strip band never drops after first paint (visible → hidden → back is the regression).
- §01 THE GROW'S TOP EDGE: at every p in [.3, 1] the ground between the rail's hem (80 at 1440x736) and the reel's top edge is ≤ 90px, and the strip's bottom is never more than 60px above the reel's top while the strip is visible (p < .4) — the vertical travel is p², the words fade only in the rail's zone (title .06–.20, strip .28–.42). A gap over 150px mid-grow is the linear travel back.
