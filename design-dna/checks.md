...
## §04 · the anchor and the conveyor
- `.dr-run--anchor`'s computed `position` is `absolute` above 900px — if it computes `relative` the rule has been moved above `.dr-run` and lost the specificity tie (the failure is silent: the card drops into flow and the pinned panel overflows).
- The anchor's viewport `left` is identical at `--sp` 0 and 1 (it must not move at all), and its `--pad`-derived padding matches a track card's exactly.
- At `--sp` 1/3, 2/3 and 1 a track card's left edge sits at the slot (anchor right + one lane) — each of 02/03/04 rests beside the anchor rather than crossing it.
- A card that has passed lands at exactly the anchor's left, fully behind it rather than half-overlapping.
- Below 900px and under `prefers-reduced-motion`: the anchor is `position: static` and the visual order top-to-bottom is 01, 02, 03, 04 (it is last in the DOM, so this needs `order: -1`).
- Scrolling 5px/frame through the whole section, no frame exceeds 20ms.
