...
- §01 THE GROW, FULL SCREEN (measured at 1440x736 and 1440x900): (a) while the strip is on screen the reel's top edge is exactly the rest gap below the strip's bottom (19px at 736, 24 at 900) — it follows at scroll's rate; (b) the reel's bottom edge is never above the fold by more than 12px at any p (the 900 sliver, closing to 0); (c) at 0.9vh of scroll the reel's rect is [0, 0, vw, vh] and its radius 0; (d) past 0.9vh it keeps that rect's size and moves with the page at 1:1 — no hold, no shrink (cut 2026-09-15); (e) the offer head's top equals the reel's bottom at every scroll past 0.9vh (gap ≤ 1px) — the flow box ends where the film does; (f) at every sample the picture's rect contains the mask's (the 16:9 video covers the box — a black band at an edge is --k under the mask); (g) the shade's rect equals the reel's within 1px at every sample.
