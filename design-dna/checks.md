...
- Pill on tiles 0/3/6 (each case's first tile; needs a REAL pointer hover, not a dispatched `mouseover`): within 700ms every `.dr-char` inside `.dr-work-see` computes a `transform` matrix whose vertical component equals −(that char's own rendered height); the pill's own rect is unchanged before/during/after; un-hover returns every `.dr-char` transform to `none`.
- An 8-step scroll sweep through the whole pinned passage: no step leaves any `.dr-work-tile` at zero width/height, `visibility: hidden`, or `opacity: 0`. A settled frame-time probe (repeated `scrollBy(0,14)`, page settled first) shows 0 frames over 34ms.
...
- At ≤900px width or under `prefers-reduced-motion: reduce`: `.dr-work-pin` is `position: static`, `.dr-work-band` computed `overflow-x` is `auto` and `scroll-snap-type` is `x mandatory`, tiles are 330×191 ±1 (⚠ `--th`'s mobile override must live on `.dr-work-stage`, the SAME element that declares `--tw`/`--pitch`/`--drift` — overriding it only on a descendant like `.dr-work-band` leaves `--tw` resolving the desktop `41.944vw` formula against the phone viewport instead, measured 283px not 330), `.dr-work-tile` computed `transform` is `none`, setting `band.scrollLeft` moves it, and the three `.dr-work-info` rows are stacked in flow (`position: static`, opacity 1, `grid-template-columns: 3ch 1fr`).
