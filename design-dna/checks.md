...
- §01 THE RELOAD (scripts/reload-screencast.mjs, then PIL per-region means): the FIRST painted frame is the light room (frame mean ≳120, never the dark room's ~34); the film region holds within ±3 from the first paint until the poster (the plate is black and becomes the film — a rise above ~35 before 800ms is the ground bleeding through a crossfade); the strip band never drops after first paint (visible → hidden → back is the regression).
