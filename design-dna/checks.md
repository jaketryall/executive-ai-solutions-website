...
- §01 THE GROW, FULL SCREEN (measured at 1440x736 and 1440x900): (a) while the strip is on screen the reel's top edge is exactly the rest gap below the strip's bottom (19px at 736, 24 at 900) — it follows at scroll's rate; (b) the reel's bottom edge is never above the fold by more than 12px at any p (the 900 sliver, closing to 0); (c) at 0.9vh of scroll the reel's rect is [0, 0, vw, vh] and its radius 0; (d) from 0.9vh to 1.0vh it stays [0, 0, vw, vh] (the hold); (e) from 1.0 to 1.35vh its centre stays at vh/2 while it shrinks (pinned in place); (f) at 1.35vh it is the column box (55→1385 at 1440), radius 24px, and the next section follows it in flow; (g) at every sample the picture's rect contains the mask's (the 16:9 video covers the box — a black band at an edge is --k under the mask); (h) the shade's rect equals the reel's within 1px at every sample.
