...
## §03 · THE WORK v2 · THE GALLERY (2026-09-18)
- Arrival (frame rect.top ≈ 736 → after 600ms): `.dr-work-wrap` scale 1.10 ±.01, `.dr-work-img` scale 1.20 ±.01; frame.top ≈ 0.6× 736: wrap scale ∈ [1.03, 1.08]; at rest: both `1`. `.dr-work-frame` rect 744×496 ±1 (ratio 1.500), centred ±1 at 1440.
- Strip: `getComputedStyle(.dr-work-stage).getPropertyValue('--rx')` reads 0 / .50 / 1.00 / 2.00 (±.02, settled) with the stage's top at 0 / −0.35vh / −0.7vh / −1.4vh; the strip's rendered card pitch is 764px (744 frame + 20 gap) at 1440 — each card's rect shifts by −rx × 764 ±2 from its rx=0 position. At rx = 1 the second card's rect equals the frame's rect ±1. Nothing renders outside the frame: `elementFromPoint` 12px outside the frame's left/right edges at mid-height is not a `.dr-work-card` descendant at rx = .5.
- Snap (real wheel input, not `window.scrollTo` alone — Lenis's Snap listens to Lenis's own `virtual-scroll` event, which only a wheel/touch gesture emits; a bare scroll-position change never fires it): landing within 260px of a card and letting the gesture stop lands scrollY on that card's exact position (±2) within ~1.8s; starting >260px from any card leaves scrollY where the gesture put it (no snap).
- Pager: at rx = 0 dot widths 48 / 8 / 8 ±.5; at rx = 1: 8 / 48 / 8. Clicking a dot scrolls (via Lenis when present, else native smooth scroll) to that card's position within 1.5s.
- Infos: at rx = 0 set 0 opacity 1, sets 1–2 opacity 0; at rx = .5 sets 0 and 1 both .125 ±.01. `--ch` (the frame's height budget) is declared on `.dr-work-gallery`, the shared ancestor of `.dr-work-frame` and `.dr-work-infos` — NOT on the frame itself, which the two once were sibling-only and could not see: assert `.dr-work-info-set`'s vertical centre equals `.dr-work-frame`'s vertical centre (±2) at every width; a regression here silently reads as `--ch` unset (`top: auto`) and the row lands at the frame's static top instead of its middle. YEAR's left edge 55 ±1 and NICHE's right edge 1385 ±1 at 1440 (464/2096 ±1 at 2560, the col-max-centred figures).
- Hover `.dr-work-pill` (needs a REAL pointer move — a JS-dispatched `mouseover` does not set `:hover`): within 700ms every `.dr-char` inside `.dr-work-see` computes a `transform` matrix with `f` = −(the char's own rendered height) (translateY(−100%)); `.dr-work-img`'s `scale` and the pill's own rect are unchanged before/during/after. Un-hover returns every `.dr-char` transform to `none`.
- Sweeping one card-to-card slide in small steps: no step shows a `.dr-work-pill` or `.dr-work-card` with zero size, `opacity: 0`, or `visibility: hidden` — nothing disappears mid-slide. Frame time settled ≤34ms across a burst sweep.
- At ≤900px width or under `prefers-reduced-motion: reduce`: `.dr-work-pin` computed `position` is `static`; `.dr-work-strip` computed `flex-direction` is `column`; `.dr-work-card` rect width/height = 1.00 ±.01 (the square now lives on the CARD, `position: relative; aspect-ratio: 1`, not the `<li>` — an aspect-ratio on the li left the absolutely-positioned card painting OVER `.dr-work-meta`, since positioned content paints after in-flow content regardless of DOM order, hiding the meta row completely); `.dr-work-wrap`/`.dr-work-img` computed `scale` is `none`; `.dr-work-infos`/`.dr-work-pager` computed `display` is `none`; `.dr-work-meta` computed `display` is `flex` and shows YEAR/NICHE; the gap between two cards' rects equals 8px + `.dr-work-meta`'s own height (measured: 64.7 = 8 + 56.7 at 390); no Snap is created (`window.lenis` stays `undefined` — there is no Lenis instance to hand a Snap on touch).
